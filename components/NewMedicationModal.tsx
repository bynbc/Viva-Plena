...
import { Pill, Loader2, Plus, X, UploadCloud, FileText } from 'lucide-react';
...
const NewMedicationModal: React.FC = () => {
  ...
  const handleAddItem = () => setItems([...items, { id: crypto.randomUUID(), name: '', dosage: '', time: '08:00' }]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // A CORREÇÃO ESTÁ AQUI: Adicionamos clinic_id em cada item
      await Promise.all(validItems.map(item => 
        push('medications', {
          clinic_id: brain.session.clinicId, // <--- O CARIMBO OBRIGATÓRIO
    ...
    <MobileModal title="Nova Prescrição" subtitle="Anexar Receita & Agendar" icon={Pill} iconColor="bg-indigo-600" onClose={() => setQuickAction(null)} footer={footer}>
      <form id="new-med-form" onSubmit={handleSave} className="space-y-6">
        ...
        <div className="space-y-2">
           ...
           <div onClick={() => fileInputRef.current?.click()} className={`w-full p-6 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center gap-2 cursor-pointer transition-all ${prescriptionFile ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-200 hover:border-indigo-400'}`}>
             {prescriptionFile ? <div className="flex items-center gap-3 text-indigo-700"><FileText size={24} /><span className="text-xs font-bold">Anexado</span></div> : <><UploadCloud size={20} /><p className="text-xs font-bold text-slate-500">Toque para enviar</p></>}
