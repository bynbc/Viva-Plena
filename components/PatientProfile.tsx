...
// Componente auxiliar extraído para evitar re-render e perda de foco
const Field = ({ label, field, placeholder, type = 'text', component = 'input', formData, handleChange, isEditing }: any) => (
...
const PatientProfile: React.FC = () => {
  ...
  }, [patient]); // Removido isEditing para evitar reset indesejado
  ...
  const handleUpdate = async () => {
    try {
      ...
      setIsEditing(false);
      // Não recarrega a página inteira, o contexto já atualiza
    } catch (err) {
      console.error(err);
  ...
  const commonProps = { formData, handleChange, isEditing };
      ...
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        ...
        {activeTab === 'personal' && (
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-4 animate-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="CPF" field="cpf" {...commonProps} />
              <Field label="RG" field="rg" {...commonProps} />
              <Field label="Data de Nascimento" field="date_of_birth" type="date" {...commonProps} />
              <Field label="Sexo" field="sex" {...commonProps} />
              <Field label="Profissão" field="profession" {...commonProps} />
              <Field label="Escolaridade" field="education_level" {...commonProps} />
              <Field label="Estado Civil" field="marital_status" {...commonProps} />
              <Field label="Nome da Mãe" field="mother_name" {...commonProps} />
              <Field label="Nome do Pai" field="father_name" {...commonProps} />
              <Field label="Naturalidade/UF" field="place_of_birth" {...commonProps} />
              <Field label="Religião" field="religion" {...commonProps} />
            </div>
          </div>
        )}
        ...
        {activeTab === 'triage' && (
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-4 animate-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Município de Origem" field="origin_city" {...commonProps} />
              <Field label="Serviço de Referência" field="reference_service" {...commonProps} />
              <Field label="Acomp. Terapêutico Anterior" field="therapeutic_accompaniment" {...commonProps} />
              <Field label="Resp. Envio Medicação" field="medication_responsible_name" {...commonProps} />
              <Field label="Contato Resp. Medicação" field="medication_responsible_contact" {...commonProps} />
              <Field label="Técnico Responsável" field="filling_technician_name" {...commonProps} />
            </div>
            <Field label="Recomendações da Equipe (Intercorrências)" field="mental_health_recommendations" component="textarea" {...commonProps} />
          </div>
        )}
        ...
        {activeTab === 'family' && (
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-6 animate-in slide-in-from-bottom-4">
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <Field label="Rua / Número" field="address_street" {...commonProps} />
                <Field label="Bairro" field="address_neighborhood" {...commonProps} />
                <Field label="Cidade / UF" field="address_city" {...commonProps} />
                <Field label="CEP" field="address_zip" {...commonProps} />
              </div>
            </div>
            ...
            <div className="border-t border-slate-100 pt-6 space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <Field label="Nome do Responsável" field="family_responsible" {...commonProps} />
                <Field label="Telefone / Contato" field="family_contact" {...commonProps} />
                <Field label="CPF do Responsável" field="family_responsible_cpf" {...commonProps} />
                <Field label="RG do Responsável" field="family_responsible_rg" {...commonProps} />
                <Field label="Vínculo/Parentesco" field="family_bond" {...commonProps} />
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'clinical' && (
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-4 animate-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Field label="Data de Admissão" field="entry_date" type="date" {...commonProps} />
              <Field label="Cartão SUS" field="sus_number" {...commonProps} />
            </div>
            <Field label="Diagnóstico (CID)" field="diagnosis" component="textarea" {...commonProps} />
            <Field label="Histórico de Dependência" field="dependence_history" component="textarea" {...commonProps} />
        ...
        {activeTab === 'financial' && (
          <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-4 animate-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {formData.payment_type === 'convenio' && <Field label="Nome do Convênio" field="insurance_name" {...commonProps} />}
              {formData.payment_type === 'particular' && <Field label="Mensalidade (R$)" field="monthly_fee" {...commonProps} />}
            </div>
          </div>
        )}
