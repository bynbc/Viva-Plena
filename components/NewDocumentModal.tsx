    ...
    <MobileModal title="Novo Documento" subtitle="Arquivar exames ou contratos" icon={FileText} iconColor="bg-blue-600" onClose={() => setQuickAction(null)} footer={footer}>
      <form id="doc-form" onSubmit={handleSave} className="space-y-6">
        <div>
          ...
          <input value={name} onChange={e => setName(e.target.value)} className="w-full p-4 bg-slate-50 text-slate-900 rounded-xl font-bold border border-slate-100 outline-none focus:border-blue-500" placeholder="Ex: Hemograma Completo" autoFocus />
        </div>

        <div>
          <label className="text-[10px] font-black text-slate-400 uppercase ml-2">Categoria</label>
          <select value={type} onChange={e => setType(e.target.value)} className="w-full p-4 bg-slate-50 text-slate-900 rounded-xl font-bold border border-slate-100 outline-none focus:border-blue-500">
            <option>Exame</option>
