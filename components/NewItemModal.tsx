  ...
  return (
    <MobileModal title="Novo Item" subtitle="Estoque & Gasto" icon={Package} iconColor="bg-blue-600" onClose={() => setQuickAction(null)}>
      <form onSubmit={handleSave} className="space-y-4">
        <input value={name} onChange={e => setName(e.target.value)} placeholder="Nome do Item" className="w-full p-4 bg-slate-50 text-slate-900 rounded-xl font-bold border border-slate-100 outline-none" />
        <input type="number" value={quantity} onChange={e => setQuantity(e.target.value)} placeholder="Quantidade" className="w-full p-4 bg-slate-50 text-slate-900 rounded-xl font-bold border border-slate-100 outline-none" />
