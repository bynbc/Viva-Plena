...
import { 
  ...
  Save, Plus, Trash2, ChevronRight, ToggleLeft, ToggleRight, Lock, Eye 
} from 'lucide-react';
...
const Settings: React.FC = () => {
  ...
  // Helper para salvar configurações no banco
          ...
          {activeSettingsSection === 'organization' && (
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10 pt-4">
                <Field 
                  label="Nome Fantasia" 
                  value={getSetting('org_name', brain.organization.name)} 
                  onChange={(v) => handleSaveSetting('org_name', v)} 
                />
                <Field 
                  label="Unidade Operacional" 
                  value={getSetting('org_unit', brain.organization.unit)} 
                  onChange={(v) => handleSaveSetting('org_unit', v)} 
                />
                <Field 
                  label="CNPJ" 
                  value={getSetting('org_cnpj', brain.organization.cnpj)} 
                  onChange={(v) => handleSaveSetting('org_cnpj', v)} 
                />
                <Field 
                  label="Sigla (Logo)" 
                  value={getSetting('org_logo', brain.organization.logo)} 
                  onChange={(v) => handleSaveSetting('org_logo', v)} 
                />
              </div>
            </div>
          )}
          ...
          {activeSettingsSection === 'notifications' && (
            <div className="space-y-12">
              ...
              <div className="space-y-6 pt-4">
                <ToggleRow 
                  label="Alertas de Ocorrência Crítica" 
                  desc="Enviar e-mail para administradores quando houver risco alto."
                  enabled={getSetting('notif_critical', true)}
                  onToggle={(v) => handleSaveSetting('notif_critical', v)}
                />
                <ToggleRow 
                  label="Resumo Diário (Daily Brief)" 
                  desc="Receber um resumo da operação todos os dias às 08:00."
                  enabled={getSetting('notif_daily', false)}
                  onToggle={(v) => handleSaveSetting('notif_daily', v)}
                />
                <ToggleRow 
                  label="Lembretes de Medicação" 
                  desc="Notificar no painel quando medicação estiver atrasada +15min."
                  enabled={getSetting('notif_meds', true)}
                  onToggle={(v) => handleSaveSetting('notif_meds', v)}
                />
              </div>
            </div>
          )}

          {/* --- SEÇÃO: SEGURANÇA E AUDITORIA (CONECTADO) --- */}
            ...
            <div className="space-y-12">
              ...
              <div className="bg-white/40 rounded-[32px] border border-white overflow-hidden">
                <div className="p-6 border-b border-white/50 flex justify-between items-center">
                  ...
                  <button className="text-[10px] font-bold text-blue-600 uppercase tracking-widest hover:underline">Exportar CSV</button>
                </div>
                <div className="divide-y divide-white/50 max-h-[400px] overflow-y-auto">
                  {auditLogs.length === 0 ? (
              ...
              <div className="mt-8 p-6 bg-rose-50 border border-rose-100 rounded-[32px] flex items-center justify-between">
                 <div>
                    <h4 className="text-rose-700 font-black text-sm">Zona de Perigo</h4>
                    <p className="text-rose-500 text-[10px] font-bold mt-1">Ações irreversíveis para a conta.</p>
                 </div>
                 <button onClick={() => addToast('Função bloqueada para segurança.', 'warning')} className="px-6 py-3 bg-white border border-rose-200 text-rose-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all shadow-sm">
                    Encerrar Contrato
                 </button>
              </div>
            </div>
              ...
              <div className="p-10 lg:p-14 glass-white border border-white rounded-[48px] shadow-2xl relative overflow-hidden group/plan">
                ...
                <div className="relative z-10">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-12 gap-8">
                    ...
                    <button className="bg-slate-950 text-white px-10 py-5 rounded-[24px] font-black text-[11px] uppercase tracking-[0.2em] hover:bg-emerald-600 transition-all shadow-xl hover:shadow-emerald-500/20 active:scale-95">Upgrade para Profissional</button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 pt-10 border-t border-slate-100">
                    <div className="space-y-3">
                      ...
                      <div className="flex items-baseline gap-3">
                        ...
                        <span className="text-base font-bold text-slate-300">/ {brain.plan.limits.patients} pacientes ativos</span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      ...
                      <div className="flex items-baseline gap-3">
                        ...
                        <span className="text-base font-bold text-slate-300">/ {brain.plan.limits.users} acessos simultâneos</span>
                      </div>
                    </div>
                  </div>
                </div>
...
// Componentes Auxiliares (Header, Field, ToggleRow) mantêm-se iguais ao anterior, já inclusos no código acima.
  ...
  <div className="space-y-3 group min-w-0">
    ...
    <div className="relative">
      <input 
        ...
        className="w-full px-7 py-5 bg-white/60 border border-slate-100 rounded-[24px] text-base font-black text-slate-900 focus:outline-none focus:bg-white focus:border-emerald-500 focus:ring-8 focus:ring-emerald-500/5 transition-all shadow-sm"
      />
      <div className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-200 group-focus-within:text-emerald-500 transition-colors">
        <Save size={20} />
      </div>
    </div>
  </div>
...
const ToggleRow = ({ label, desc, enabled, onToggle }: any) => (
  <div className="flex items-center justify-between p-6 glass bg-white/40 border border-white rounded-[24px] hover:bg-white transition-all">
    ...
    <button 
      ...
      className={`text-4xl transition-colors ${enabled ? 'text-emerald-500' : 'text-slate-300'}`}
    >
