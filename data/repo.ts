...
export const repo = {
  ...
  async createPatient(clinicId: string, userId: string, payload: Partial<Patient>) {
    ...
    if (payload.status === 'active' && (count || 0) >= limit) {
      throw { code: 'PLAN_LIMIT_PATIENTS', message: `Limite atingido (${limit}/${limit}).` };
    }
  ...
  async updatePatient(clinicId: string, patientId: string, patch: Partial<Patient>) {
  ...
  // --- OUTROS MÓDULOS ---
  async listRecords(clinicId: string) {
    return supabase.from('records').select(`*, patient:patients(name)`).eq('clinic_id', clinicId).order('created_at', { ascending: false });
  },
  async createRecord(clinicId: string, userId: string, payload: Partial<DailyRecord>) {
    return supabase.from('records').insert([{ ...payload, clinic_id: clinicId, created_by: userId }]).select().single();
  },
  async deleteRecord(clinicId: string, id: string) {
    return supabase.from('records').delete().eq('id', id).eq('clinic_id', clinicId);
  },

  async listOccurrences(clinicId: string) {
    return supabase.from('occurrences').select(`*, patient:patients(name)`).eq('clinic_id', clinicId).order('created_at', { ascending: false });
  },
  async createOccurrence(clinicId: string, userId: string, payload: Partial<Occurrence>) {
    return supabase.from('occurrences').insert([{ ...payload, clinic_id: clinicId, created_by: userId }]).select().single();
  },
  async deleteOccurrence(clinicId: string, id: string) {
    return supabase.from('occurrences').delete().eq('id', id).eq('clinic_id', clinicId);
  },

  async listAgenda(clinicId: string) {
    return supabase.from('agenda').select(`*, patient:patients(name)`).eq('clinic_id', clinicId).order('start_at');
  },
  async createAgendaEvent(clinicId: string, userId: string, payload: Partial<AgendaEvent>) {
    return supabase.from('agenda').insert([{ ...payload, clinic_id: clinicId, created_by: userId }]).select().single();
  },
  async deleteAgendaEvent(clinicId: string, id: string) {
    return supabase.from('agenda').delete().eq('id', id).eq('clinic_id', clinicId);
  },

  async listDocuments(clinicId: string) {
    return supabase.from('documents').select(`*, patient:patients(name)`).eq('clinic_id', clinicId).order('created_at', { ascending: false });
  },
  async createDocument(clinicId: string, userId: string, payload: Partial<DocumentRecord>) {
    return supabase.from('documents').insert([{ ...payload, clinic_id: clinicId, created_by: userId }]).select().single();
  },
  async deleteDocument(clinicId: string, id: string) {
    return supabase.from('documents').delete().eq('id', id).eq('clinic_id', clinicId);
  },

  async listSettings(clinicId: string) {
    return supabase.from('settings').select('*').eq('clinic_id', clinicId);
  },
  async updateSetting(clinicId: string, key: string, value: any) {
  ...
  async listClinicUsers(clinicId: string) {
    const { data, error } = await supabase
      .from('clinic_users')
      .select(`permissions, user:app_users(id, username, role, is_active)`)
      .eq('clinic_id', clinicId);
  ...
  async createAppUser(clinicId: string, adminUserId: string, userData: { username: string, password_hash: string, role: 'ADMIN' | 'NORMAL', permissions: UserPermissions }) {
    const { data: clinic } = await repo.getClinic(clinicId);
  ...
  // --- MEDICAÇÃO ---
  async listMedications(clinicId: string) {
  ...
  async createMedication(clinicId: string, userId: string, data: any) {
    return await supabase.from('medications').insert([{ ...data, clinic_id: clinicId }]);
  },

  async updateMedication(clinicId: string, id: string, data: any) { 
    return await supabase.from('medications').update(data).eq('id', id).eq('clinic_id', clinicId); 
  },

  async administerMedication(id: string, userId: string, username: string) {
    return await supabase.from('medications').update({
      status: 'administered',
  ...
  async deleteMedication(clinicId: string, id: string) {
    return supabase.from('medications').delete().eq('id', id).eq('clinic_id', clinicId);
  },

  // --- TRANSAÇÕES ---
  async createTransaction(clinicId: string, data: any) { 
    return await supabase.from('transactions').insert([{ ...data, clinic_id: clinicId }]); 
  },
  async updateTransaction(clinicId: string, id: string, data: any) { 
    return await supabase.from('transactions').update(data).eq('id', id).eq('clinic_id', clinicId); 
  },
  async deleteTransaction(clinicId: string, id: string) { 
    return supabase.from('transactions').delete().eq('id', id).eq('clinic_id', clinicId); 
  },
